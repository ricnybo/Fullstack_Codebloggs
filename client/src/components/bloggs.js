...
function Bloggs() {
    ...
    return (
        <>
            <CommentModal show={showCommentModal} onHide={() => setShowCommentModal(false)} postId={postIdForComment} refreshPosts={fetchPosts} />
                                ...
                                <div key={post._id}>
                                    <div className="bloggs-post-group" >
                                        <div style={{ display: 'flex' }}> {/* Add a flex container */}
                                            ...
                                            <div style={{ marginLeft: '10px', flexGrow: "1" }}> {/* Add some space between the initials and the rest of the content */}
                                                <div className="bloggs-post-content" >
                                                    <div>Poster: {post.user_id.first_name} {post.user_id.last_name}</div>
                                                    <br />
                                                    <div >{post.content}</div>
                                                </div>
                                                <div className="bloggs-post-info">
                                                    <span>{new Date(post.time_stamp).toLocaleString()}</span>
                                                    <span className=""><img src="./img/like vector icon.png" alt="Likes" style={{ width: "30px", height: "30px" }} onClick={() => handlePostLike(post._id, post.user_id._id)} /> {post.likes}</span>
                                                </div>
                                                ...
                                                {post.comments.map((comment) => (
                                                    <div key={comment._id}>
                                                        ...
                                                        <div className="bloggs-post-comment-info">
                                                            ...
                                                            <span className=""><img src="./img/like vector icon.png" alt="Likes" style={{ width: "30px", height: "30px" }} onClick={() => handleCommentLike(comment._id, comment.user_id._id)} /> {comment.likes}</span>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
